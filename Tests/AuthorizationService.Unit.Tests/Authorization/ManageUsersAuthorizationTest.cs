...
{
    [TestClass]
    public class ManageUsersAuthorizationTest
    {
        ...
        public void OnAuthorization_TokenExistAndValid_Correct()
        {
            ...
            AuthService.Setup(x => x.Registrate(token, new List<Role> { Role.ManageUsers })).Returns(new AuthResult { StatusCode = 200, UserId = userid, SessionId = sessionId });
            var actionDescriptor = new Mock<ControllerActionDescriptor>();

            var filter = new ManageUsersAuthorizationFilter(AuthService.Object);
            ...
            AuthService.Verify(x => x.Registrate(token, new List<Role> { Role.ManageUsers }), Times.Once);
        ...
        public void OnAuthorization_TokenNotSpecified_ReturnUnauthorized()
        {
            ...
            AuthService.Verify(x => x.Registrate(token, new List<Role> { Role.ManageUsers }), Times.Never);
            var result = context.HttpContext.Items.TryGetValue("userId", out var contextUserId);
        ...
        public void OnAuthorization_ServiceReturnUnauthorized_ReturnUnauthorized()
        {
            ...
            AuthService.Setup(x => x.Registrate(token, new List<Role> { Role.ManageUsers })).Returns(new AuthResult { StatusCode = 401, Message = authMessage });
            var actionDescriptor = new Mock<ControllerActionDescriptor>();
        ...
        public void OnAuthorization_ServiceReturnForbidden_ReturnForbidden()
        {
            ...
            AuthService.Setup(x => x.Registrate(token, new List<Role> { Role.ManageUsers })).Returns(new AuthResult { StatusCode = 403, Message = authMessage });
            var actionDescriptor = new Mock<ControllerActionDescriptor>();
